...
class ClassCard(Card):
    ...
    def __init__(self,name):
        ...
        # Stats
        self._stats = [
                'name','copies','player','enemy','every_enemy','target','text',
    ...
    def get_description_text(self):
        ...
        prop_targets = _get_default_target_properties()
        ...
        prop_targets.update(self.target)
        ...
        if prop_targets:
            description += _target_interpreter(prop_targets)
        ...
        return format_text_linebreaks(' '.join(description),limit = 26)

""" Text interpreters """

def _target_interpreter(prop):
    description = []

    if prop['damage_per_turn']:
        description += ["Deal {} damage at start of target's turn".format(
            prop['damage_per_turn'])]
    if prop['block_per_turn']:
        description += ["Target gains {} block at start of turn".format(
            prop['block_per_turn'])]
    if prop['damage_per_discarded_illusion']:
        description += ["Discard all illusions on target.\
                Deal {} damage for each illusion discarded".format(
                    prop['damage_per_discarded_illusion'])]
    if prop['block_per_discarded_illusion']:
        description += ["Discard all illusions on target.\
                Gain {} block for each illusion discarded".format(
                    prop['block_per_discarded_illusion'])]
    if prop['draw_per_discarded_illusion']:
        description += ["Discard all illusions on target.\
                Draw {} card for each illusion discarded".format(
                    prop['draw_per_discarded_illusion'])]
    if prop['heal_per_discarded_illusion']:
        description += ["Discard all illusions on target.\
                Heal {} HP for each illusion discarded".format(
                    prop['heal_per_discarded_illusion'])]
    if prop['move_illusion']:
        description += ['Move an illusion to a new target']
    if prop['damage']:
        description += ["Deal {} damage to target".format(prop['damage'])]
    if prop['block']:
        description += ["Target gains {} block".format(prop['block'])]
...
def _special_boon_interpreter(prop):
    ...
    if prop['reduce_card_draw']:
        description += ['Draw {} less card next turn'.format(prop['reduce_card_draw'])]
    if prop['no_card_draw']:
        description += ['Do not draw cards next turn']
...
def _parse_conditional(prop):
    ...
    if prop[0] == 'attack':
        when += 'play an Attack,'
    if prop[0] == 'attack_this_turn':
        when += 'play an Attack this turn,'
    if prop[0] == 'skill':
        when += 'play a Skill,'
    if prop[0] == 'skill_this_turn':
        when += 'play a Skill this turn,'
    if prop[0] == 'block':
        when += 'you gain block,'
    if prop[0] == 'lose hp':
        when += 'lose HP,'
...
def _get_default_target_properties():
    return copy.deepcopy({
            'damage_per_turn': 0,
            'block_per_turn': 0,
            'damage_per_discarded_illusion': 0,
            'block_per_discarded_illusion': 0,
            'draw_per_discarded_illusion': 0,
            'heal_per_discarded_illusion': 0,
            'move_illusion':  0,
            'damage':  0,
            'block':  0,
            'mult_block': 0,
